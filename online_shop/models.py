...
class ProductInventory(models.Model):
    ...
    def __str__(self):
        return f"mahsulot soni: {self.quantity} ta"
...
class Product(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    inventory_id = models.ForeignKey(ProductInventory, on_delete=models.CASCADE, related_name='inventory', null=True, blank=True)
...
@receiver(pre_save, sender=CardItem)
def correct_price(sender, **kwargs):
    cart_items = kwargs['instance']
    price_of_product = Product.objects.get(id=cart_items.product_id.id)
    cart_items.price = cart_items.quantity * float(price_of_product.price)
    total_cart_items = CardItem.objects.filter(user=cart_items.user)
